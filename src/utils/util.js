...
export default {
  ...
  dateFormat: (date) => {
    ...
    const time = `${year}-${month}-${mydate} ${hours}:${minutes}:${seconds}`
    return time
  },
  ...
  fileUrlDownload(fileUrl, fileName) {
    ...
    const url = process.env.VUE_APP_FILE_URL + fileUrl // 若是不完整的url则需要拼接baseURL
